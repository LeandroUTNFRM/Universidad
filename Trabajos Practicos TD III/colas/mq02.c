...
int main() {
   ...
   mqd = mq_open(MQ_PATH, O_WRONLY | O_CREAT, 0666, &attr);//se cambió O_RDONLY por O_WRONLY
   if (mqd < 0) {
...
//Respuesta:
//El error es que se estaba abriendo la cola de mensajes para lectura en vez de escritura
//Cambiando O_RDONLY por O_WRONLY en la apertura de la cola de mensajes se soluciona el problema
